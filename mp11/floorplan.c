...
/*****************************************INTRO PARAGRAPH********************************
 *  In floorplan.c, we have to complete functions:
 * init_slicing_tree which takes in a node and a number and creates a left-skewed slicing
 * tree that has all vertical cutlines. It initializes leaf nodes based on an array of
 * predefined modules
 *
 * get_total_resource which takes the root pointer and returns the value of the sum of
 * all resources in the entire tree
 *
 * is_leaf_node which takes a node pointer and returns 1 if the node is a leaf node
 *
 * is_internal_node which takes a node pointer and returns a 1 if the node is an internal
 * node
 *
 * is_in_subtree which takes node pointers a and b and determines whether b is within the
 * subtree of a
 *
 * postfix_traversal which takes the root node, a counter variable, and an array of
 * expressions and fills the expression array with the postfix expression of the entire
 * tree
 *
 * rotate which takes a leaf node and flips the width and height of the module
 *
 * recut which takes an internal node and cuts it the other way from which it is initially
 * cut (v->h or h->v)
 *
 * swap_module which takes two nodes and swaps the modules in those nodes
 *
 * swap_topology which takes two nodes and swaps their location on the tree, along with any
 * subtrees
 *
 */
...
int is_leaf_node(node_t* ptr) {
  ...
  if(ptr->module!=NULL) { return 1; } //if ptr has a module, return 1
  return 0;
}
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
  if(ptr->cutline!=UNDEFINED_CUTLINE) { return 1; }  //if cutline is not undefined, return 1
  return 0;
}
...
int is_in_subtree(node_t* a, node_t* b) {
  ...
  if(a==NULL) { return 0; }
  if(a==b) { return 1; }
  while(b!=NULL){ //loop up through tree from node b to find if a is a parent of b
    if(b->parent==a){ //return 1 if it is
      return 1;
    }
    b = b->parent;  //increment b
  }
  return 0; //if b==NULL
}
...
void rotate(node_t* ptr) {
  // TODO:
  int temp = ptr->module->w;  //temp = width
  ptr->module->w = ptr->module->h;  //width = height
  ptr->module->h = temp;  //height = temp
  return;
}
...
void recut(node_t* ptr) {
  ...
  // TODO:
  if(ptr->cutline==V){ ptr->cutline = H; }  //if cutline is vertical, make it horizontal
  else { ptr->cutline = V; }  //else make it vertical
  return;
}
...
void swap_module(node_t* a, node_t* b) {
  ...
  // TODO:
  module_t* temp = a->module;  //temp module = module a
  a->module = b->module;  //module a = module b
  b->module = temp; //module b = temp
  return;
}
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
  node_t* temp;
  if(a->parent->left == a){ //if a is a left node from a->parent
    if(b->parent->right==b) { a->parent->left = b; b->parent->right = a; }  //if b is right node, swap
    if(b->parent->left==b) { a->parent->left = b; b->parent->left = a; }  //if b is left node, swap
    temp = a->parent; //swap parents
    a->parent = b->parent;
    b->parent = temp;
  }
  else if(a->parent->right == a){ //if a is a right node from a->parent
    if(b->parent->right==b) { a->parent->right = b; b->parent->right = a; }  //if b is right node, swap
    if(b->parent->left==b) { a->parent->right = b; b->parent->left = a; }  //if b is left node, swap
    temp = a->parent; //swap parents
    a->parent = b->parent;
    b->parent = temp;
  }
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  // TODO:
  postfix_traversal(ptr->left,nth,expression);  //base case already given, so recursively call prt->right and left
  postfix_traversal(ptr->right,nth,expression);
  if(ptr->module!=NULL){ //fill expression[n] with module and cutline values
...
int get_total_resource(node_t* ptr)
{
  ...
  if(ptr==NULL) { return 0; } //base case if empty tree
  if(ptr->left==NULL && ptr->right==NULL) { return ptr->module->resource; } //base case if leaf, return resource
  sum+=get_total_resource(ptr->left); //recursively call and add return value to sum
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  node_t* ptr = (node_t*)malloc(sizeof(node_t));  //create new node

  if(n==num_modules-1){ //base case
    ptr->cutline = UNDEFINED_CUTLINE; //initalize cutline
    ptr->parent = par;  //ptr parent = par
    ptr->left = ptr->right = NULL;  //leaf node
    ptr->module = &modules[n];  //initialize module
    return ptr;
  }

  ptr->module = NULL; //initialize ptr module
  ptr->cutline = V; //initialize cutline V
  ptr->parent = par;  //initialize parent = par

  ptr->right = (node_t*)malloc(sizeof(node_t)); //create right node
  ptr->right->cutline = UNDEFINED_CUTLINE;  //right is leaf node
  ptr->right->right = ptr->right->left = NULL;
  ptr->right->parent = ptr; //right parent is ptr
  ptr->right->module = &modules[n]; //initialize module

  ptr->left = NULL;
  ptr->left = init_slicing_tree(ptr,n+1);  //recursive call for left node
  return ptr;
}
